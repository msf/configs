...
const credentialsPath = resolve(home, ".pi/agent/mcp-auth.json");
...
Uses Pi's mode-0600 MCP OAuth store. Run /mcp-login slack in Pi when auth expires. Never prints tokens.`);
  process.exit(2);
}
...
function readPiEntry() {
  const store = readJson(credentialsPath);
  const entry = store.servers?.slack;
  if (!entry) throw new Error("Pi Slack OAuth is missing. Run /mcp-login slack in interactive Pi.");
  return { store, entry };
}

function makePiOAuthProvider() {
  ...
  return {
    ...
    get clientMetadata() {
      return {
        ...
        response_types: ["code"],
        client_name: "pi-mcp-slack",
      };
    },

    clientInformation() {
      return readPiEntry().entry.clientInformation ?? { client_id: clientId };
    },

    tokens() {
      const stored = readPiEntry().entry.tokens;
      if (!stored?.accessToken) return undefined;
      const tokens = { access_token: stored.accessToken, token_type: "Bearer" };
      if (stored.refreshToken) tokens.refresh_token = stored.refreshToken;
      if (stored.scope) tokens.scope = stored.scope;
      if (stored.expiresAt) tokens.expires_in = Math.max(0, Math.floor((stored.expiresAt - Date.now()) / 1000));
      return tokens;
    },

    saveTokens(tokens) {
      const { store, entry } = readPiEntry();
      entry.tokens = {
        accessToken: tokens.access_token,
        refreshToken: tokens.refresh_token ?? entry.tokens?.refreshToken,
        expiresAt: tokens.expires_in ? Date.now() + tokens.expires_in * 1000 : undefined,
        scope: tokens.scope ?? entry.tokens?.scope,
      };
      writeJson(credentialsPath, store);
    },

    discoveryState() { return readPiEntry().entry.discoveryState; },
    ...
    redirectToAuthorization() {
      throw new Error("Pi Slack OAuth is missing or expired. Run /mcp-login slack in interactive Pi.");
    },
...
async function withClient(fn) {
  ...
  const transport = new StreamableHTTPClientTransport(new URL(serverUrl), {
    authProvider: makePiOAuthProvider(),
  });
