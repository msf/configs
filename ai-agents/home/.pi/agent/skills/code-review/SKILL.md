...
### 2. Spawn the Pi code-reviewer agent

- Use the `code-reviewer` subagent defined in `~/.pi/agent/agents/code-reviewer.md`. Its command restrictions are prompt-enforced because Pi agent frontmatter supports tool allowlists, not per-command denials; verify its conduct and output.
- Launch one `code-reviewer` per PR. Split within a PR only for genuinely independent areas.
- Pass the paths of `coding` and each relevant language/domain skill. The isolated agent must read those files itself before reviewing; do not paste whole skills into the task.
- Build the task from `references/subagent-prompt-template.md`, including ticket, PR-discussion, stack, and cross-repo context already gathered.
- Treat the result like a junior's PR: re-read every surfaced location and only keep findings you personally understand with high confidence.
